...
string Controller::send_notification(char *cmd){
...
long Controller::getFileCreationTime(char *path,int first){
    struct stat attr;
    stat(path, &attr);
    ifstream filestr;
...
int Controller::traverseFS(char* folder,int first_time,Model &model,View &view){
	DIR *dir;
	struct dirent *ent;
        ...
        if( it != changed_times.end()){
            if(it->second < lastModified){
                if( it->first.substr(it->first.length() - 2) != ".." && it->first.substr(it->first.length() - 2) != "/."){
                    ...
                    traverseFS(folder,1,model,view);
                }
            }
            else{
